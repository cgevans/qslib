...
`qslib-server` provides an optional HTTP API and SSE event stream for QuantStudio
...
The server owns one managed SCPI connection for API requests and
...
(Following paragraph generated by LLM)
...
| `--max-access` | `Controller` | Highest SCPI access allowed for API requests. |
...
The HTTP service is opt-in. If it is unavailable before a read is
...
(Code example below generated by LLM)
