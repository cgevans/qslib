                ...
                loop {
                    ...
                    while let Some(end) = pending.find('\n') {
                        ...
                        if command == "POW ON" {
                            ...
                            // per-topic buffer if the connection actor stops
...
async fn queued_start_uses_the_exact_package_version_accepted_by_etag() {
    ...
    // (Text below generated by LLM)
    // Hold an earlier server operation in the actor so the accepted start
...
#[tokio::test]
async fn subscriptions_are_drained_while_server_operations_run() {
