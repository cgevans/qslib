...
fn bench_message_parsing(c: &mut Criterion) {
    ...
    group.bench_function("simple_message", |b| {
        b.iter(|| {
            MessageResponse::try_from(black_box(&simple_msg[..])).unwrap()
        });
    });

    // LED status message benchmark
    let led_msg = b"MESSage LEDStatus Temperature:56.1434 Current:9.19802 Voltage:3.40984 JuncTemp:72.7511\n";
    group.bench_function("led_status", |b| {
        b.iter(|| {
            MessageResponse::try_from(black_box(&led_msg[..])).unwrap()
        });
    });
    ...
    group.bench_function("base64_message", |b| {
        b.iter(|| {
            MessageResponse::try_from(black_box(&base64_msg[..])).unwrap()
        });
    });
