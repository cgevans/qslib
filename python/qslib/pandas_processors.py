...
class PandasProcessor(metaclass=ABCMeta):
    @abstractmethod
    def process_scoped(
...
@attrs.define()
class SmoothWindowMean(PandasProcessor):
    """
...
@attrs.define()
class SmoothEMWMean(PandasProcessor):
    """
...
class NormToMeanPerWell(PandasProcessor):
    """
...
@dataclass(init=False)
class SubtractByMeanPerWell(PandasProcessor):
    """
...
@dataclass
class NormToMaxPerWell(PandasProcessor):
    """
    A Processor that divides the fluorescence reading for each (filterset, well) pair
