...
_NO_SERVER_RESULT = object()


def _raise_if_server_auth_error(error: BaseException) -> None:
    ...
    the caller whenever the HTTP request would otherwise fall back to SCPI.
...
def _ensure_connection(level: AccessLevel = AccessLevel.Observer) -> Any:
    def wrap(func):
        ...
        def wrapped(m: Machine, *args: Any, **kwargs: Any) -> Any:
            server_result = m._server_dispatch(func.__name__, args, kwargs, required_access=level)
            if server_result is not _NO_SERVER_RESULT:
...
class Machine:
    ...
    def connect(self) -> None:
        ...
        automatic methods into the qslib-server HTTP API; it does not
    ...
    def _server_for(
        ...
        *,
        ...
        """Select qslib-server for a resource without opening a SCPI connection.
    ...
    def _server_dispatch(
        ...
        *,
        ...
        Returning ``_NO_SERVER_RESULT`` means the permanent direct-SCPI
        ...
        if name in {"read_file", "write_file", "download_dir", "list_files", "_get_log_from_byte"}:
            ...
            server = self._server_for("runs", mutation=is_mutation)
        else:
            server = self._server_for("instrument", mutation=is_mutation)
        if server is None:
            return _NO_SERVER_RESULT
        ...
        try:
            ...
            if is_mutation:
                if name != "write_file":
                    raise
                return _NO_SERVER_RESULT
            log.debug("qslib-server read failed for %s; using direct SCPI", name, exc_info=True)
            return _NO_SERVER_RESULT
        return _NO_SERVER_RESULT
    ...
    def upload_zip_as_files(self, context_path: str, zipbytes: bytes, *, fast: bool = True) -> bool:
        ...
        """
        server = self._server_for("files", mutation=True) if fast else None
        if server is None:
            return False
    ...
    def get_running_protocol(self) -> Protocol:
        ...
        if server is not None:
            ...
            try:
                ...
                _raise_if_server_auth_error(error)
                log.debug("qslib-server protocol query failed; using direct SCPI", exc_info=True)
    ...
    def generate_random_key(self) -> str:
        ...
        """
        server = self._server_for("instrument", mutation=True)
        if server is not None:
            self._require_access_cap(AccessLevel.Controller)
            try:
    ...
    def get_zone_count(self) -> int:
        ...
        """
        server = self._server_for("instrument")
        if server is not None:
            self._require_access_cap(AccessLevel.Observer)
            try:
                return int(server.instrument_status()["zone_count"])
                ...
                _raise_if_server_auth_error(error)
                log.debug("qslib-server zone-count query failed; using direct SCPI", exc_info=True)
        with self.ensured_connection(AccessLevel.Observer):
    ...
    def status(self) -> RunStatus:
        ...
        if server is not None:
            ...
            try:
                ...
                _raise_if_server_auth_error(error)
                log.debug("qslib-server run status failed; using direct SCPI", exc_info=True)
        with self.ensured_connection(AccessLevel.Observer):
            out = self.run_command_bytes(RunStatus.command())
            return RunStatus.from_bytes(out)
    ...
    def drawer_position(self) -> DrawerPosition:
        ...
        if server is not None:
            ...
            try:
                ...
                _raise_if_server_auth_error(error)
                log.debug("qslib-server drawer status failed; using direct SCPI", exc_info=True)
        with self.ensured_connection(AccessLevel.Observer):
    ...
    def cover_position(self) -> CoverPosition:
        ...
        if server is not None:
            ...
            try:
                ...
                _raise_if_server_auth_error(error)
                log.debug("qslib-server cover status failed; using direct SCPI", exc_info=True)
        with self.ensured_connection(AccessLevel.Observer):
    ...
    def power(self) -> bool:
        ...
        if server is not None:
            ...
            try:
                ...
                _raise_if_server_auth_error(error)
                log.debug("qslib-server power query failed; using direct SCPI", exc_info=True)
        with self.ensured_connection(AccessLevel.Observer):
    ...
    def current_run_name(self) -> str | None:
        ...
        if server is not None:
            ...
            try:
                ...
                _raise_if_server_auth_error(error)
                log.debug("qslib-server current-run query failed; using direct SCPI", exc_info=True)
        with self.ensured_connection(AccessLevel.Observer):
