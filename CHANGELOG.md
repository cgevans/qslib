...
(Sections below through the first two bullets under "Other" generated by LLM)

### qslib-server and HTTP clients
...
- Added Rust and Python HTTP clients and automatic `Machine` dispatch for supported high-level operations, with bounded direct-SCPI fallback only when the optional server is unavailable before submission; HTTP authorization failures remain visible.
...
- Added the typed `RemainingTimeQuery`, `QSConnection::get_run_remaining_time`, and HTTP-server `remaining_time_s` status field.
