...
def server(tmp_path: Path):
    ...
    # Keep the server's output: discarding it left CI failures with nothing to
    # diagnose. Not under tmp_path, which is the --file-root the file tests list.
    ...
    try:
        while time.monotonic() < deadline:
            if process.poll() is not None:
                raise RuntimeError(f"qslib-server exited with {process.returncode}\n{server_output()}")
            try:
            ...
            raise TimeoutError(f"qslib-server actor did not become ready\n{server_output()}")
