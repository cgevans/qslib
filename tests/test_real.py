...
def _retire_stale_server(m: Machine, expected_sha: str) -> None:
    ...
    with m.ensured_connection(AccessLevel.Controller):
        ...
        # The instrument image has no pkill, pgrep, ps or killall, and kill is
        # only a shell builtin, so match the process name in /proc and kill the
        # PIDs from the shell. Matching on comm rather than the command line
        # avoids killing the SYST:EXEC shell running this. Backticks, not
        # $(...): the InstrumentServer expands ${...} and $(...) itself before
