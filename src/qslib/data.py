...
class FilterDataReading:
    def __init__(self, filterxmlstring: Union[str, bytes], time=None, set_temperatures='auto'):
        ...
        self.attribs = {k.lower(): v for k, v in zip(
        ...
        self.well_fluorescence = np.fromstring(fxml.xpath("//WellData/text()")[0], sep=' ')
    ...
    def well_temperatures(self) -> np.ndarray:
        return self.temperature[np.tile(np.arange(0, 
                len(self.attribs['temperature'])).repeat(
    ...
    def well_set_temperatures(self) -> np.ndarray:
        if self.set_temperatures is not None:
            return self.set_temperatures[np.tile(np.arange(0, len(self.set_temperatures)).repeat(
    ...
    def to_lineprotocol(self):
        ...
        wr = [(r, c) for r in _UPPERS[0:self.plate_rows] for c in range(1,self.plate_cols+1)]

        for (r, c), f, tr, ts in zip(wr, self.well_fluorescence, self.well_temperatures, self.well_set_temperatures):
